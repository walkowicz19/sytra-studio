...
<aside class="sidebar">
  ...
  <!-- Storage location -->
  <div class="storage-row" data-tooltip={cacheDir} data-tooltip-align="left">
    <div class="storage-text">
      <span class="storage-label">{t('sidebar.storage')}</span>
      <span class="storage-path text-mono">{shortDir(cacheDir) || '…'}</span>
    </div>
    <button
    ...
    >
      <i class="bi bi-folder2-open" style="font-size:13px"></i>
    </button>
  </div>

  <div class="memory-row">
    <label for="select-memory-limit">Main memory</label>
    <select id="select-memory-limit" value={memoryChoice} onchange={changeMemoryLimit} aria-label="Main memory limit">
  ...
  <!-- Language -->
  <div class="lang-row">
    <i class="bi bi-globe2" style="font-size: 13px; color: var(--color-ink-ghost)"></i>
    <select
      class="lang-select"
  ...
  <div class="mode-row">
    <div class="mode-text">
      <span class="mode-label">{t('sidebar.advancedMode')}</span>
      <span class="mode-hint">{uiMode.advanced ? t('sidebar.allSettings') : t('sidebar.guided')}</span>
    </div>
...
<style>
  ...
  .storage-text { display: flex; flex-direction: column; gap: 1px; min-width: 0; }
  .storage-label { font-size: 10px; font-weight: 600; letter-spacing: 0.08em; text-transform: uppercase; color: var(--color-ink-ghost); }
  .storage-path { font-size: 10px; color: var(--color-ink-subtle); white-space: nowrap; overflow: hidden; text-overflow: ellipsis; }

  .memory-row { display: flex; flex-direction: column; gap: 4px; padding: 0 var(--space-1); }
  .memory-row label { font-size: 10px; font-weight: 600; letter-spacing: 0.08em; text-transform: uppercase; color: var(--color-ink-ghost); }
  .memory-row select { width: 100%; height: 30px; padding: 0 var(--space-2); background: var(--color-surface); color: var(--color-ink); border: 1px solid var(--color-border); border-radius: var(--radius-sm); font: 11px var(--font-sans); }
  .memory-row select:focus-visible { outline: 2px solid var(--color-brand); outline-offset: 1px; }
  ...
  .lang-select:hover { border-color: var(--color-border-strong); color: var(--color-ink); }
  .lang-select:focus { border-color: var(--color-brand); }
  ...
  .mode-label { font-size: 11px; font-weight: 600; letter-spacing: 0.06em; text-transform: uppercase; color: var(--color-ink); }
  .mode-hint { font-size: 10px; color: var(--color-ink-ghost); }
