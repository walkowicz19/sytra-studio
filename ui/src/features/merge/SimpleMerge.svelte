<script lang="ts">
  ...
  import { run, resetRun, pushToast, hwStore } from '../../store.svelte'
  import { t } from '../../i18n.svelte'
  import { api } from '../../api'
  import { startWatchedOp } from '../shared/startWatchedOp'
  ...
  onMount(async () => {
    try {
      ...
      pushToast('error', `Failed to list local models: ${e instanceof Error ? e.message : String(e)}`)
    }
  })
  ...
  async function pickLocalModel(index: number) {
    try {
      ...
      pushToast('error', `Model picker failed: ${e instanceof Error ? e.message : String(e)}`)
    }
  }

  const vramMb = $derived(hwStore.info ? hwStore.info.vram_mb : 0)

  function getModelVramStatus(modelPath: string): { label: string; status: 'ok' | 'host' | 'unknown' } {
    ...
    if (match) {
      ...
      if (vramMb != null && neededMb <= vramMb) {
        return { label: `Fits VRAM (${match.size_gb} GB)`, status: 'ok' }
      } else {
        return { label: `Host-memory merge required (${match.size_gb} GB)`, status: 'host' }
      }
    }
  ...
  async function start() {
    ...
    try {
      await startWatchedOp('merge', () => api.startMerge(config as never))
    } catch (e) {
      run.status = 'error'
          ...
          {#each models as _, i}
            ...
            <div class="model-row-group">
              ...
              <div class="model-meta-row">
                ...
                {:else if vStat.status === 'host'}
                  <span class="badge badge-warning"><i class="bi bi-memory"></i> {vStat.label}</span>
                {/if}
              </div>
            </div>
          {/each}
