<script lang="ts">
  import { api } from '../../api'
  import { run, resetRun, applyTelemetry, pushToast, mergeFormStore } from '../../store.svelte'
  ...
  async function startMerge() {
    ...
    try {
      await startWatchedOp('merge', () => api.startMerge(config))
      if (!('__TAURI_INTERNALS__' in window)) simulateMerge()
    } catch (e: unknown) {
      run.status = 'error'
