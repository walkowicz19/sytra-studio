...
- Set `SYTRA_LLAMA_SERVER`, `SYTRA_VLLM_COMMAND`, or `SYTRA_ENGINE_COMMAND` when a binary is not on `PATH`. `SYTRA_VLLM_TENSOR_PARALLEL_SIZE` can override the detected GPU count.
